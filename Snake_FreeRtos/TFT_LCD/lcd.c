...
//LCD的画笔颜色和背景色	   
uint32_t lcdFRONT_COLOR=LcdColor_RED;	//画笔颜色
uint32_t lcdBACK_COLOR =LcdColor_BLACK;  	//背景色
...
uint8_t	 LCD_FS=24;	//字体大小, 一个字符宽度为半个字体大小
uint16_t LCD_CurPosX;	//当前位置X
uint16_t LCD_CurPosY;	//当前位置Y
...
{										   
	...
	HAL_Delay(1);			//原来是delay_us(5);//暂时注释掉，函数LCD_ReadReg()没有被调用过
...
void LCD_Display_Dir(uint8_t dir)
{
	...
	LCD_W =lcddev.width;		//LCD 宽度,会在LCD_Display_Dir()函数里初始化
	LCD_H =lcddev.height;		//LCD 高度
}	 
...
#ifdef  LCD_ID_9341
void LCD_Init9341()//lcddev.id=0x9341的初始化
{
	LCD_WR_REG(0xCF);
	LCD_WR_DATA(0x00);
	LCD_WR_DATA(0xC1);
...
#ifdef LCD_ID_5310
void LCD_Init5310()//lcddev.id=0x5310的初始化
{
	LCD_WR_REG(0xED);
	LCD_WR_DATA(0x01);
	LCD_WR_DATA(0xFE);
...
#ifdef LCD_ID_5510
void LCD_Init5510()//lcddev.id=0x5510的初始化
{
	...
	LCD_WR_REG(0x1100);
	HAL_Delay(1);		//原来是 delay_us(120);
	LCD_WR_REG(0x2900);
}
#endif

#ifdef LCD_ID_1963
void LCD_Init1963()//lcddev.id=0x1963的初始化
{
...
///==========确定LCD ID
	//尝试9341 ID的读取
#if AUTO_DETECT_LCD_ID			//自动检测LCD ID
	LCD_WR_REG(0XD3);
...
//		LCD_Init1963();
	...
	//初始化完成以后,提速, 直接操作寄存器, 用于FSMC连接的LCD
	...
	LCD_Display_Dir(0);		//默认为竖屏,设置Width和Height
...
{
	if ((fontSize != 12) || (fontSize != 16) ||(fontSize != 24) ||(fontSize != 32))
		return;
	LCD_FS=fontSize;
}
...
{  							  
	...
	{   
		...
		{			    
			if(temp & 0x80)
				LCD_Fast_DrawPoint(x,y,lcdFRONT_COLOR);
			else if(mode==0)
				LCD_Fast_DrawPoint(x,y,lcdBACK_COLOR);
			temp<<=1;
			y++;
			if(y>=lcddev.height)
				return;		//超区域了
...
{  
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			...
			{
				if(mode&0X80)
					LCD_ShowChar(x+(size/2)*t,y,'0',mode&0X01);
				else
					LCD_ShowChar(x+(size/2)*t,y,' ',mode&0X01);
 				continue;
			}else
				enshow=1;
